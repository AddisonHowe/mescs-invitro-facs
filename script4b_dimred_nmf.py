...
OUTDIR = f"out/4b_dimred_nmf/{KEY}/dec{TRANSITION_IDX}"
...
if LOG_NORMALIZE:
    ...
    print(np.min(xfacs), np.max(xfacs))
    x = logicle(xfacs, None, a=1)
...
nmf = NMF(n_components=len(GENES))
...
if FIT_ON_SUBSET:
    ...
    # Fit NMA on all of the cells of interest.
...
for pc_set in pc_sets:
    ...
    np.save(f"{outdir}/components.npy", nmf.components_)
...
#     print(f"PC{i+1}: {100*nmf.explained_variance_ratio_[i]:.5f}%")
...
if 2 in MAKEPLOTS:
    ...
    for cond_idx, cond_name in CONDITIONS.items():
        ...
        plt.tight_layout()
        plt.savefig(
            f"{IMGDIR}/nmf_decision_{TRANSITION_IDX}_{cond_name}_scatter.pdf", 
            bbox_inches='tight'
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            f"{IMGDIR}/nmf_decision_{TRANSITION_IDX}_{cond_name}_density.pdf", 
            bbox_inches='tight'
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            f"{IMGDIR}/nmf_decision_{TRANSITION_IDX}_{cond_name}_kde.pdf", 
            bbox_inches='tight'
        )
        plt.close()
...
if 3 in MAKEPLOTS:
    ...
    for cond_idx, cond_name in CONDITIONS.items():
        ...
        plt.tight_layout()
        plt.savefig(
            f"{IMGDIR}/nmf_decision_{TRANSITION_IDX}_{cond_name}_histograms.pdf", 
            bbox_inches='tight'
        )
        plt.close()
...
if 6 in MAKEPLOTS:
    ...
    for cond_idx, cond_name in CONDITIONS.items():
        ...
        plt.savefig(
            f"{IMGDIR}/nmf_dec{TRANSITION_IDX}_{cond_name}_sig_hist.pdf", 
